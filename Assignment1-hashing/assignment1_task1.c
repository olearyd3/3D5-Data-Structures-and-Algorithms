...
//creates a new element for the new string to be stored, as well as how many times it occurred
element* create_new_element(char* name) {
    //create a variable and allocate it memory. Give key and value NULL and 0
	element* temp = (element*)malloc(sizeof(element));
	temp->key = NULL;
	temp->value = 0;
    //add one to the number of terms
    num_terms++;
    //now copy name to key and add 1 to value ((tried to use strcpy(), but it wouldn't work so used strdup() instead
