...
#define ARRAY_SIZE 59 //a large array size (best if prime)
#define R_coefficient 31 //prime coefficient to give diff weights to diff bits
...
//defining global variables for num_terms and collisions - easier this way
int num_terms = 0;
int collisions = 0;
int capacity = -1;
...
//returns the sum of integer values of chars in a string
int hash2(char* s) {
    int hash = 0;
    while(*s) {
        //multiplied by R_coefficient to add different weights to different bits
        hash = (R_coefficient*hash + *s)%ARRAY_SIZE;
        s++;
    }
    return hash;
}
